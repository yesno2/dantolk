 ...
 * 	@ {Object} err : 인증 실패 또는 정보가 없음 또는 잘못된 요청 정보
 **/
exports.getBranches = function(authInfo, option){
	return new Promise((resolve, reject) => {
		try{
			let github = auth.auth(authInfo);
...
//exports.getRepository('', '').then((result) => {
//exports.getRepositoryAll('', '').then((result) => {
//exports.getRepository('', '', '').then((result) => {
//exports.getBranches('', '', '').then((result) => {
/*exports.getCommit({
	userName: '', 
	password: ''
}, {
	user: '',
	repo: '',
	per_page: 100}).then((result) =>{*/
//exports.getCommit('', '', '', '').then((result) => {
